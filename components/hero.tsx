"use client";

import { motion } from "framer-motion";

import { Stats } from "@/components/stats";

import { fadeIn } from "@/lib/variants";

export const Hero = () => {
  return (
    <section
      id="home"
      className="relative h-212.5 w-full pt-56.25 pb-63.5 mb-12 lg:mb-28 bg-hero bg-cover bg-right lg:bg-center bg-no-repeat text-white"
    >
      <div className="container text-center">
        <motion.h1
          variants={fadeIn("up", 0.2)}
          initial="hidden"
          whileInView="show"
          viewport={{ once: false, amount: 0.2 }}
          className="mb-7.5 lg:max-w-222 mx-auto text-2xl lg:text-[64px] font-semibold lg:leading-tight"
        >
          Modern Living Elevate Your Home Design
        </motion.h1>
        <motion.p
          variants={fadeIn("up", 0.4)}
          initial="hidden"
          whileInView="show"
          viewport={{ once: false, amount: 0.2 }}
          className="mb-7.5 lg:mb-16.25 max-w-156.25 mx-auto lg:text-xl"
        >
          Transform your space with timeless pieces, crafted for comfort, style,
          and everyday elegance.
        </motion.p>
        <motion.button
          variants={fadeIn("up", 0.6)}
          initial="hidden"
          whileInView="show"
          viewport={{ once: false, amount: 0.2 }}
          type="button"
          className="btn btn-cta mb-40 lg:mb-48.5"
        >
          Shop Now
        </motion.button>
        <div>
          <Stats />
        </div>
      </div>
    </section>
  );
};
